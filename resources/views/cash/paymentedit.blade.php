...
@section('content')
    <div class="container">
        ...
        <form method="POST" action="{{route('updatepayment', ['id' => $datos->first()->id_cash])}}">
            @csrf
            <div class="col-md-5 mb-3">
                ...
                <select class="form-select" id="validationCustom04" name="payment" required>
                    ...
                    <option value="1" {{ $datos->first()->payment_type == '1' ? 'selected' : '' }}> Efectivo</option>
                    <option value="2" {{ $datos->first()->payment_type == '2' ? 'selected' : '' }}> Transferencia</option>
                    <option value="3" {{ $datos->first()->payment_type == '3' ? 'selected' : '' }}> Yape</option>
                    <option value="4" {{ $datos->first()->payment_type == '4' ? 'selected' : '' }}> Plin</option>
                    <option value="5" {{ $datos->first()->payment_type == '5' ? 'selected' : '' }}> Tarjeta Visa</option>
                </select>
